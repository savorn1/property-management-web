...
import type { Building, CreateBuildingPayload, UpdateBuildingPayload } from '~/composables/useBuildings'
...
const filter = reactive<{ propertyId: number | undefined; name: string }>({
...
const createFields = computed<FieldDef[]>(() => [
  ...
  { name: 'code' },
  { name: 'totalFloors', label: 'Total floors', type: 'number', min: 0 },
...
watch(sort, load)
watch(() => filter.propertyId, load)

const hasActiveFilter = computed(() => filter.propertyId !== undefined || filter.name !== '')
